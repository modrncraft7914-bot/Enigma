...
class Criptogtaphi:
    ...
    def ids_in_text(self,mas,alph,sps):
        ot = ""
        с = 0
    ...
    def add_data_infile(self,name,data):
        with open(name,'w') as f:
            f.seek(0)
            f.write(data)
    def read_data_infile(self,name,fnd_ids=False,ids=False):
    ...
    def generete_key(self,is_save_in_file=False,name=False):
        out = ""
        ...
        if is_save_in_file:
            self.add_data_infile(name,out)

        return out

    def cript(self,key,text,alph,is_return_text=False,is_save_message=False,name=False):
        ad = key.split('|') # el
        #1 act - rotors
        arotors = []
        adr = ad[0].split('#')
        ...
        if is_return_text:
            print(nt_ob_s)
            out = self.ids_in_text(out,alph,nt_ob_s)
        if is_save_message:
            self.add_data_infile(name,message)
        return out

    def encript(self,key,text,alph,is_return_text=False,is_save_message=False,name=False):
        ad = key.split('|') # el
        #1 act - rotors
        arotors = []
        adr = ad[0].split('#')
...
t = Criptogtaphi(alphabet,10,3)
t.generete_key(True,"enigma_key")
kl = t.read_data_infile("enigma_key")
k = t.cript(kl,message,alphabet,True)

sv = k
k5 = t.encript(kl,k,alphabet,True)
